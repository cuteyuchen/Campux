...
/** 开启 LLM 总结时：每个投稿编号换行展示 */

export const BATCH_CAPTION_SEPARATOR_LLM = "\n";

/** 未开启 LLM 总结时：编号之间 4 个空格，同一行发布 */

export const BATCH_CAPTION_SEPARATOR_PLAIN = "    ";



/**

 * 把批次内每条稿件已渲染好的配文合并成一条说说的正文。

 * - separator 默认换行（LLM 总结开启时）

 * - 未开启 LLM 时传 4 空格，同一行拼接

 * 空/纯空白的 caption 会丢弃（已按 position 排序）。

 */

export function joinBatchCaptions(captions: string[], separator: string = BATCH_CAPTION_SEPARATOR_LLM): string {
 ...
 * 手动立即发布：冲刷租户当前所有 collecting 且有稿件的批次。

 * 与 sweeper 共用租户锁，避免并发双重入队。

 */

export async function flushCollectingBatches(
  queue: RuntimeQueue,
  tenantId: string,
  actorId: string | null,
  logger?: FastifyBaseLogger,
  ...
  const candidates = await prisma.publishBatch.findMany({
    where: { tenantId, status: "collecting", imageCount: { gt: 0 } },
    select: { id: true, imageCount: true },
    orderBy: { createdAt: "asc" },
  });
  ...
  await lockTenantBatch(tenantId, async () => {
    for (const batch of candidates) {
      const fresh = await prisma.publishBatch.findUnique({
        where: { id: batch.id },
        select: { status: true, imageCount: true },
      });
      if (!fresh || fresh.status !== "collecting" || fresh.imageCount <= 0) {
        continue;
      }
      await prisma.publishBatch.update({
        where: { id: batch.id },
      ...
      const items = await prisma.publishBatchItem.findMany({
  ...
  for (const batchId of toFlush) {
    logger?.info({ batchId, tenantId }, "collecting batch flushed manually");
    await enqueueBatchPublishFanout(queue, tenantId, batchId, actorId);
  }
